...
with open("sample.json") as json_file:
    ...
    for i in range(0, 3):
        h_timer = json_data["featureConfigs"]["features"][5]["bgp"]["bgpNeighbours"]["bgpNeighbours"][i][
            "holdDownTimer"]
        k_alive = json_data["featureConfigs"]["features"][5]["bgp"]["bgpNeighbours"]["bgpNeighbours"][
            i]["keepAliveTimer"]

        h_timer += k_alive
        json_data["featureConfigs"]["features"][5]["bgp"]["bgpNeighbours"]["bgpNeighbours"][i][
            "holdDownTimer"] = h_timer

with open("sample.json", "w") as json_d:
    json_d.write(json.dumps(json_data))
